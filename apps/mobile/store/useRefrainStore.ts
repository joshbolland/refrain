...
export const useRefrainStore = create<RefrainState>((set, get) => ({
  ...
  replaceRecording(recording) {
    set((state) => {
      const index = state.recordings.findIndex((entry) => entry.id === recording.id);
      const next = [...state.recordings];
      if (index === -1) {
        next.unshift(recording);
      } else {
        next[index] = recording;
      }
      return { recordings: next.sort((a, b) => b.updatedAt - a.updatedAt) };
    });
  },
  ...
  async deleteRecording(id) {
    const { collectionAssignments, collections, recordings } = get();
    ...
    await recordingRepo.deleteRecording(target);
  },
