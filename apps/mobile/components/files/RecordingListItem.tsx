  ...
  const content = (
    ...
    >
      <View className="flex-row items-center justify-between">
        <View className="flex-row items-center flex-wrap" style={{ gap: 8 }}>
          ...
          {recordingNeedsRepair(recording) ? (
            <View className="rounded-full px-3 py-1" style={{ backgroundColor: '#FFF6E7' }}>
              <Text className="text-xs font-semibold uppercase tracking-[0.14em]" style={{ color: '#B46812' }}>
                Needs repair
              </Text>
            </View>
          ) : null}
      ...
      <Text className="mt-2 text-sm text-muted/90" numberOfLines={2}>
        {recordingNeedsRepair(recording)
          ? 'Original file exists only on the source device until repaired.'
