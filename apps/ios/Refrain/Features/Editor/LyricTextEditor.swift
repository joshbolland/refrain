...
struct LyricEditorViewportPolicy: Equatable {
    let bottomClearance: CGFloat
    let editingLineReserve: CGFloat

    init(
        bottomClearance: CGFloat = 12,
        editingLineReserve: CGFloat = Theme.editorLineHeight
    ) {
        self.bottomClearance = bottomClearance
        self.editingLineReserve = editingLineReserve
    }

    var trailingScrollInset: CGFloat {
        editingLineReserve
    }
    ...
    func visibleBottom(
        ...
        let editingReserve = isActivelyEditing ? editingLineReserve : 0
        return contentOffsetY + max(0, viewportHeight - bottomClearance - editingReserve)
    }

    func maximumContentOffset(
        contentHeight: CGFloat,
        viewportHeight: CGFloat,
        contentInsetTop: CGFloat,
        contentInsetBottom: CGFloat
    ) -> CGFloat {
        let minimumOffset = -contentInsetTop
        return max(minimumOffset, contentHeight + contentInsetBottom - viewportHeight)
    }
    ...
    func targetContentOffset(
        ...
        let visibleTop = currentOffsetY + bottomClearance
        let visibleBottom = visibleBottom(
            contentOffsetY: currentOffsetY,
            viewportHeight: viewportHeight,
            isActivelyEditing: isActivelyEditing
        )

        var targetOffset = currentOffsetY
        if caretMaxY > visibleBottom {
            targetOffset += caretMaxY - visibleBottom
        } else if caretMinY < visibleTop {
            targetOffset -= visibleTop - caretMinY
        } else {
        ...
        return min(max(targetOffset, minimumOffset), maximumOffset)
    }
}
...
struct LyricTextViewRepresentable: UIViewRepresentable, Equatable {
    ...
    private func updateInsets(for textView: UITextView) {
        let policy = viewportPolicy
        let desiredTextContainerInset = UIEdgeInsets(
            top: Theme.editorPaddingTop,
            left: Theme.editorHorizontalPadding,
            bottom: policy.bottomClearance,
            right: Theme.editorHorizontalPadding
        )
        let desiredContentInset = UIEdgeInsets(
            top: 0,
            left: 0,
            bottom: policy.trailingScrollInset,
            right: 0
        )
        let desiredScrollIndicatorInsets = UIEdgeInsets.zero
    ...
    private var viewportPolicy: LyricEditorViewportPolicy {
        LyricEditorViewportPolicy(editingLineReserve: lineHeight)
    }
    ...
    class Coordinator: NSObject, UITextViewDelegate, UIGestureRecognizerDelegate {
        ...
        func scrollCaretIntoView(for textView: UITextView, animated: Bool) {
            ...
            let targetOffsetY = parent.viewportPolicy.targetContentOffset(
                caretMinY: caretRect.minY,
                caretMaxY: caretRect.maxY,
                currentOffsetY: textView.contentOffset.y,
                viewportHeight: textView.bounds.height,
                contentHeight: textView.contentSize.height,
                contentInsetTop: textView.contentInset.top,
                contentInsetBottom: textView.contentInset.bottom,
                isActivelyEditing: textView.isFirstResponder
            )

            guard abs(targetOffsetY - textView.contentOffset.y) > 0.5 else {
                return
            }
